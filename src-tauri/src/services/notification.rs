...
fn send_platform_notification(
    ...
    // Toast COM objects are thread-affine and !Send, so the Toast must be
    // created and shown on the main thread where the Tauri event loop runs.
    // The on_activated callback is delivered through the main thread's
    // message pump.
    app.run_on_main_thread(move || {
        ...
        // show() registers the toast with Windows and returns immediately.
        // The Toast struct must outlive the toast's on-screen lifetime
        // because it owns the COM event source.  We leak it to keep the
        // on_activated callback alive — the memory cost is one Toast per
        // notification (a few hundred bytes), acceptable for the
        // infrequent notification volume.
        if let Err(e) = toast.show() {
            log::warn!("notification:windows-show-failed error={e}");
        }
        std::mem::forget(toast);
    })
    .map_err(|e| {
        log::warn!("notification:windows-run-on-main-thread-failed error={e}");
        e.to_string()
    })?;
